...
- `feature/*` branches are integrated into `develop` through pull requests with a linear history. Once the pull request's checks pass, a maintainer rebases the branch onto `develop` if needed, fast-forwards `develop` locally and pushes; GitHub marks the pull request as merged. The web merge buttons are not used, because platform-generated merges stamp the account's platform identity as committer. A repository ruleset blocks direct pushes, force pushes and deletions on `main` and `develop` for everyone except repository administrators, who perform these fast-forward merges (decision of the Product Owner, 2026-09-23).
